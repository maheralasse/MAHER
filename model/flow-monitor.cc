...
uint32_t
FlowMonitor::CalculateAllocationSize (const SSRecord *ssRecord, const ServiceFlow *serviceFlow)
{
...
uint32_t
FlowMonitor::BandwidthCalculator (uint32_t bytesToRequest)
{
  Ptr<Packet> packet;
          ...
          if (serviceFlow->HasPackets (MacHeaderType::HEADER_TYPE_GENERIC))
            {
              ...
              bandwidth_required = bytesToRequest;
              break;
            }
        }
    }
 
  return bandwidth_required;
}

void
FlowMonitor::SendBandwidthRequest (uint8_t uiuc, uint16_t allocationSize)
{
  Ptr<SubscriberStationNetDevice> ss = m_device->GetObject<SubscriberStationNetDevice> ();

  uint32_t bytesToRequest = 0;
  ServiceFlow *serviceFlow = BandwidthCalculator (bytesToRequest);
...
void
FlowMonitor::ProcessBandwidthRequest (const BandwidthRequestHeader &bwRequestHdr)
{
  Ptr<BaseStationNetDevice> bs = m_device->GetObject<BaseStationNetDevice> ();
...
void
FlowMonitor::SetSubframeRatio (void)
{
...
uint32_t
FlowMonitor::GetSymbolsPerFrameAllocated (void)
{
  Ptr<BaseStationNetDevice> bs = m_device->GetObject<BaseStationNetDevice> ();
