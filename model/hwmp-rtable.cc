...
HwmpRtable::DeleteProactivePath ()
{
  ...
  m_root.bandwidth_required.clear ();                     ///////////
  m_root.processing_delay_required.clear ();           /////////////
}
void
...
HwmpRtable::LookupMaximumTheBestBandwidth (Mac48Address OriginatorAddress)          ////////////////
{
 uint8_t m_bandwidth_required = 0;

 for (std::map<Mac48Address, ReactiveRoute>::iterator i = m_routes.begin (); i != m_routes.end (); i++)
    {
    
   if (( m_bandwidth_required>= i->second.bandwidth_required )&&(i->second.retransmitter == OriginatorAddress))
    {
      m_bandwidth_required = i->bandwidth_required;    
    }}
    return LookupResult ();
  ...
  return LookupReactiveExpired (OriginatorAddress);
}
...
HwmpRtable::LookupReactiveExpired (Mac48Address destination)
{
  ...
  return LookupResult (i->second.retransmitter, i->second.interface, i->second.metric, i->second.seqnum,
                 i->second.whenExpire - Simulator::Now (),i->second.m_bandwidth_required, i->second.m_processing_delay_required);  /////////////
}
HwmpRtable::LookupResult
...
HwmpRtable::LookupProactiveExpired ()
{
  return LookupResult (m_root.retransmitter, m_root.interface, m_root.metric, m_root.seqnum,
                  m_root.whenExpire - Simulator::Now (),m_root.bandwidth_required,m_root.processing_delay_required);         /////////////
}
...
HwmpRtable::LookupResult::operator== (const HwmpRtable::LookupResult & o) const
{
  return (retransmitter == o.retransmitter && ifIndex == o.ifIndex && metric == o.metric && seqnum
          == o.seqnum && bandwidth_required == o.bandwidth_required && processing_delay_required == o.processing_delay_required );
}
HwmpRtable::LookupResult::LookupResult (Mac48Address r, uint32_t i, uint32_t m, uint32_t s, Time l,uint8_t b ,uint8_t p) :
  retransmitter (r), ifIndex (i), metric (m), seqnum (s), lifetime (l) ,bandwidth_required (b) ,processing_delay_required (p)
{
}
bool
HwmpRtable::LookupResult::IsValid () const
{
  return !(retransmitter == Mac48Address::GetBroadcast () && ifIndex == INTERFACE_ANY && metric == MAX_METRIC
           && seqnum == 0 && bandwidth_required ==0  && processing_delay_required==0);
}
} // namespace ns3
