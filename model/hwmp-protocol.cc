...
HwmpProtocol::ReceivePreq (IePreq preq, Mac48Address from, uint32_t interface, Mac48Address fromMp, uint32_t metric,uint8_t processing_delay_required,uint8_t bandwidth_required)
{
  ...
  if (
    ...
    (
     ...
     HwmpRtable::LookupResult result = m_rtable->LookupMaximumTheBestBandwidth (prep.GetOriginatorAddress ());
    {
      m_rtable->AddReactivePath (
        result.GetOriginatorAddress (),
        from,
        interface,
        result.GetMetric (),
        MicroSeconds (preq.GetLifetime () * 1024),
        result.GetOriginatorSeqNumber ()
        result.GetProcessing_delay_required (),                                /////////////////////
        result.GetBandwidth_required ()                                       /////////////////////
        );        
